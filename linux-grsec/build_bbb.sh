...
if [ ! -d ${KERN_SOURCE_DIR} ]; then
    ...
    if [ ! -f ${KERNEL_BUILD_DIR}/${KERNEL_SOURCE_ARCHIVE} ]; then
        ...
        GRSEC_PATCH=grsecurity-3.1-4.4.8-201604201957.patch
        wget -c https://raw.githubusercontent.com/slashbeast/grsecurity-scrape/master/test/${GRSEC_PATCH}
    fi
...
cp -v ${KERN_SOURCE_DIR}/arch/arm/boot/zImage ${KERNEL_DEPLOY}/boot/vmlinuz-4.4.8-grsec
cp -v ${KERN_SOURCE_DIR}/System.map ${KERNEL_DEPLOY}/boot/system.map-4.4.8-grsec
cp -v ${KERN_SOURCE_DIR}/.config ${KERNEL_DEPLOY}/boot/config-4.4.8-grsec
